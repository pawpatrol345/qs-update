...
async function fillForm(page, data, log) {
  ...
  // Save confirmation - with double-confirmation workaround for QuikSkope bug
  const step5Start = Date.now();
  console.log("✅ Confirming...");
  log("STEP 5: First save attempt");

  try {
    log('⏳ Waiting for confirm modal (first save)...');
    ...
    if (!modalReady) {
      ...
      throw new Error('Confirm modal did not appear after save');
    }

    // Click first confirmation (this saves but has a bug in QS)
    log('  Clicking confirm button (first save - will have QS bug)...');
    await page.evaluate(() => {
      const btn = document.querySelector('#confirmLoadData');
      if (btn) {
        btn.disabled = false;
    ...
    log('⏳ Waiting for first save to complete...');
    await Promise.race([
    ...
    const afterFirstSave = page.url();
    log(`✓ First save completed - Current URL: ${afterFirstSave}`);
    log('⚠️  Note: Due to QuikSkope bug, first save may not persist');
    
    // Now go back to Create Load to trigger second save (workaround for QS bug)
    log('');
    log('🔄 STEP 6: Second save (QuikSkope bug workaround)');
    ...
    await page.waitForSelector('a#createLoadByForm', { timeout: 5000 });
    ...
    await wait(1500);
    log('✓ Create Load clicked - back on form page');
    
    // Verify form has data
    log('  Verifying form still has data...');
    const formHasData = await page.evaluate(() => {
      ...
      return { loadNum, driverName, hasData: loadNum.length > 0 && driverName.length > 0 };
    });
    
    log(`  Load#: ${formHasData.loadNum}, Driver: ${formHasData.driverName}`);
    if (!formHasData.hasData) {
      log('⚠️  WARNING: Form data is empty! Data may have been lost.');
    } else {
      log('✓ Form data still present');
    }
    
    // Click save button directly (second save - this is the actual save)
    log('  Clicking save button (SECOND TIME - ACTUAL SAVE)...');
    await page.waitForSelector('#saveAndContinuePreviewLoad', { visible: true, timeout: 3000 });
    await page.click('#saveAndContinuePreviewLoad');
    await wait(300);
    log('✓ Save button clicked');
    
    // Wait for modal (second time)
    log('⏳ Waiting for confirm modal (second save)...');
    ...
    if (!secondModalReady) {
      log('❌ Second modal did not appear');
      throw new Error('Second confirmation modal did not appear');
    }

    // Click confirm (second time - this is the ACTUAL save that works)
    log('  Clicking confirm button (FINAL SAVE - THIS ONE WORKS)...');
    await page.evaluate(() => {
      const btn = document.querySelector('#confirmLoadData');
      if (btn) {
        btn.disabled = false;
    ...
    log(`✓ FINAL confirmation complete (${Date.now() - step5Start}ms) - URL: ${finalUrl}`);
    console.log("✅ Load ACTUALLY saved (after double-save workaround for QS bug)!");
    ...
    log(`❌ Save/Confirm error: ${error.message}`);
